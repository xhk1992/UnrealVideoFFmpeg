...
bool FVideoStreamWmfMp4Writer::Initialize(const TCHAR* Filename)
{
	CHECK_HR(MFCreateSinkWriterFromURL(Filename, nullptr, nullptr, Writer.GetInitReference()));
	UE_LOG(LogVideoStream, Verbose, TEXT("Initialised Mp4Writer for %s"), Filename);
	return true;
}

TOptional<DWORD> FVideoStreamWmfMp4Writer::CreateAudioStream(const FString& Codec, const AVEncoder::FAudioConfig& Config)
{
	GUID Format;
	...
	{
		UE_LOG(LogVideoStream, Error, TEXT("FWmfMp4Writer doesn't support codec %s"), *Codec);
		return {};
	}
...
TOptional<DWORD> FVideoStreamWmfMp4Writer::CreateVideoStream(const FString& Codec, const AVEncoder::FVideoConfig& Config)
{
	GUID Format;
	if (Codec == "h264")
	{
...
bool FVideoStreamWmfMp4Writer::Start()
{
	CHECK_HR(Writer->BeginWriting());
	return true;
}

bool FVideoStreamWmfMp4Writer::Write(const AVEncoder::FMediaPacket& InSample, DWORD StreamIndex)
{
	...
	UE_LOG(LogVideoStream, VeryVerbose, TEXT("stream #%d: time %.3f, duration %.3f%s"), StreamIndex, Sample.GetTime().GetTotalSeconds(), Sample.GetDuration().GetTotalSeconds(), InSample.IsVideoKeyFrame() ? TEXT(", key-frame") : TEXT(""));

	return true;
}

bool FVideoStreamWmfMp4Writer::Finalize()
{
	CHECK_HR(Writer->Finalize());
	UE_LOG(LogVideoStream, VeryVerbose, TEXT("Closed .mp4"));
