...
DECLARE_LOG_CATEGORY_EXTERN(LogWmfRingBuffer, Log, VeryVerbose);
DEFINE_LOG_CATEGORY(LogWmfRingBuffer);

WINDOWSPLATFORMFEATURES_START

void FVideoStreamWmfRingBuffer::Push(AVEncoder::FMediaPacket&& Sample)
{
	check(MaxDuration != 0);
...
void FVideoStreamWmfRingBuffer::Cleanup()
// removes a/v samples grouped by the oldest key-frame period
{
	...
	UE_LOG(LogWmfRingBuffer, VeryVerbose, TEXT("%d samples, %.3f s, %.3f - %.3f:%.3f"),
		Samples.Num(),
...
void FVideoStreamWmfRingBuffer::PauseCleanup(bool bPause)
{
	bCleanupPaused = bPause;
}

FTimespan FVideoStreamWmfRingBuffer::GetDuration() const
{
	if (Samples.Num() == 0)
	{
		return 0;
...
TArray<AVEncoder::FMediaPacket> FVideoStreamWmfRingBuffer::GetCopy()
{
	FScopeLock Lock(&Mutex);
...
void FVideoStreamWmfRingBuffer::Reset()
{
	FScopeLock Lock(&Mutex);
	Samples.Empty();
}
