...
int32 GCustomFrameGrabberFrameLatency = 0;
static FAutoConsoleVariableRef CVarFrameGrabberFrameLatency(
	TEXT("customframegrabber.customframelatency"),
	GCustomFrameGrabberFrameLatency,
...
FCustomViewportSurfaceReader::FCustomViewportSurfaceReader(EPixelFormat InPixelFormat, FIntPoint InBufferSize)
{
...
FCustomViewportSurfaceReader::~FCustomViewportSurfaceReader()
{
	BlockUntilAvailable();
...
void FCustomViewportSurfaceReader::Initialize()
{
	check(!AvailableEvent);
	AvailableEvent = FPlatformProcess::GetSynchEventFromPool();
}

void FCustomViewportSurfaceReader::Resize(uint32 Width, uint32 Height)
{
	ReadbackTexture.SafeRelease();

	FCustomViewportSurfaceReader* This = this;
...
void FCustomViewportSurfaceReader::BlockUntilAvailable()
{
	if (AvailableEvent)
	{
...
void FCustomViewportSurfaceReader::Reset()
{
	if (AvailableEvent)
	{
...
void FCustomViewportSurfaceReader::ResolveRenderTarget(FCustomViewportSurfaceReader* RenderToReadback, const FTexture2DRHIRef& SourceBackBuffer, TFunction<void(FColor*, int32, int32)> Callback)
{
...
FCustomFrameGrabber::FCustomFrameGrabber(TSharedRef<FSceneViewport> Viewport, FIntPoint DesiredBufferSize, EPixelFormat InPixelFormat, uint32 NumSurfaces)
{
	...
	FrameGrabLatency = GCustomFrameGrabberFrameLatency;

	// Ensure textures are setup
	FlushRenderingCommands();
}

FCustomFrameGrabber::~FCustomFrameGrabber()
{
...
void FCustomFrameGrabber::StartCapturingFrames()
{
	...
	OnBackBufferReadyToPresent = FSlateApplication::Get().GetRenderer()->OnBackBufferReadyToPresent().AddRaw(this, &FCustomFrameGrabber::OnBackBufferReadyToPresentCallback);
}

bool FCustomFrameGrabber::IsCapturingFrames() const
{
	return State == EFrameGrabberState::Active;
}

void FCustomFrameGrabber::CaptureThisFrame(FCustomFramePayloadPtr Payload)
{
...
void FCustomFrameGrabber::StopCapturingFrames()
{
	if (!ensure(State == EFrameGrabberState::Active))
	{
		return;
...
void FCustomFrameGrabber::Shutdown()
{
	State = EFrameGrabberState::Inactive;
...
bool FCustomFrameGrabber::HasOutstandingFrames() const
{
...
TArray<FCustomCapturedFrameData> FCustomFrameGrabber::GetCapturedFrames()
{
	TArray<FCustomCapturedFrameData> ReturnFrames;
...
void FCustomFrameGrabber::OnBackBufferReadyToPresentCallback(SWindow& SlateWindow, const FTexture2DRHIRef& BackBuffer)
{
	...
	FCustomFramePayloadPtr Payload;
	...
	if (FrameGrabLatency != GCustomFrameGrabberFrameLatency)
	{
		FlushRenderingCommands();
		for (FResolveSurface& Surface : Surfaces)
		{
	...
	FCustomViewportSurfaceReader* PrevFrameTarget = &Surfaces[PrevCaptureIndex].Surface;
...
void FCustomFrameGrabber::OnFrameReady(int32 BufferIndex, FColor* ColorBuffer, int32 Width, int32 Height)
{
	...
	{
		FCustomCapturedFrameData ResolvedFrameData(TargetSize, Surface.Payload);
