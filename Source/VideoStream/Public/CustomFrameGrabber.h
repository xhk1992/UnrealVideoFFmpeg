...
struct FCustomViewportSurfaceReader
{
	/** Constructor */
	FCustomViewportSurfaceReader(EPixelFormat InPixelFormat, FIntPoint InBufferSize);

	/** Destructor */
	~FCustomViewportSurfaceReader();
	 ...
	 */
	void ResolveRenderTarget(FCustomViewportSurfaceReader* RenderToReadback, const FTexture2DRHIRef& BackBuffer, TFunction<void(FColor*, int32, int32)> Callback);
...
struct ICustomFramePayload
{
	virtual ~ICustomFramePayload() {}
...
typedef TSharedPtr<ICustomFramePayload, ESPMode::ThreadSafe> FCustomFramePayloadPtr;

/** Structure representing a captured frame */
struct FCustomCapturedFrameData
{
	FCustomCapturedFrameData(FIntPoint InBufferSize, FCustomFramePayloadPtr InPayload) : BufferSize(InBufferSize), Payload(MoveTemp(InPayload)) {}

	FCustomCapturedFrameData(FCustomCapturedFrameData&& In) : ColorBuffer(MoveTemp(In.ColorBuffer)), BufferSize(In.BufferSize), Payload(MoveTemp(In.Payload)) {}
	FCustomCapturedFrameData& operator=(FCustomCapturedFrameData&& In) { ColorBuffer = MoveTemp(In.ColorBuffer); BufferSize = In.BufferSize; Payload = MoveTemp(In.Payload); return *this; }
...
private:
	FCustomCapturedFrameData(const FCustomCapturedFrameData& In);
	FCustomCapturedFrameData& operator=(const FCustomCapturedFrameData& In);
};
...
public:
	 ...
	 */
	FCustomFrameGrabber(TSharedRef<FSceneViewport> Viewport, FIntPoint DesiredBufferSize, EPixelFormat InPixelFormat = PF_B8G8R8A8, uint32 NumSurfaces = 3);
...
public:
	...
	void CaptureThisFrame(FCustomFramePayloadPtr Payload);
...
public:
	...
	TArray<FCustomCapturedFrameData> GetCapturedFrames();
...
private:
	...
	/** Non-copyable */
	FCustomFrameGrabber(const FCustomFrameGrabber&);
	FCustomFrameGrabber& operator=(const FCustomFrameGrabber&);
	...
	TArray<FCustomCapturedFrameData> CapturedFrames;
	...
	struct FResolveSurface
	{
		...
		FCustomFramePayloadPtr Payload;
		FCustomViewportSurfaceReader Surface;
	};
	...
	TArray<FCustomFramePayloadPtr> RenderThread_PendingFramePayloads;
